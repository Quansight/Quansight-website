...
Because the numba engine is still very new and experimental, support does not include for now:

- Support for Apache Arrow arrays
- Datetime/Timedelta
- String Arrays
  - We are planning on adding support for this either through supporting Apache Arrow backed Series/DataFrames or through the new numpy [string dtype](https://numpy.org/neps/nep-0055-string_dtype.html)
...
df = pd.DataFrame({"a": np.random.randn(size), "b": np.random.randn(size), "c": np.random.randn(size)})
```
...
```
    ...
    CPU times: user 312 ms, sys: 6.93 ms, total: 319 ms
    Wall time: 321 ms
...
```
    ...
    CPU times: user 985 µs, sys: 15 µs, total: 1e+03 µs
    Wall time: 1e+03 µs
...
```python
%timeit df.apply(f, engine="numba", axis=1)
```

    330 ms ± 24.6 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)
...
```
    ...
    276 ms ± 5.16 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)
    1.98 s ± 99.3 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)

```python
f = lambda x: x[10]
%timeit df.apply(f, engine="numba", axis=0)
%timeit df.apply(f, engine="python", axis=0)
```

    1.52 ms ± 227 µs per loop (mean ± std. dev. of 7 runs, 1 loop each)
    218 µs ± 27.6 µs per loop (mean ± std. dev. of 7 runs, 1,000 loops each)
...
```
    ...
    25.3 ms ± 2.3 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)
    67.2 ms ± 615 µs per loop (mean ± std. dev. of 7 runs, 10 loops each)
...
```
    ...
    343 ms ± 19.2 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)
...
```
    ...
    795 µs ± 81.6 µs per loop (mean ± std. dev. of 7 runs, 1 loop each)
    1.09 ms ± 8.36 µs per loop (mean ± std. dev. of 7 runs, 1,000 loops each)
...
%timeit df.apply(f, engine="numba", axis=1, raw=False)
```

    326 ms ± 38.2 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)
...
```python
%timeit df.apply(raw_f, engine="numba", axis=1, raw=True, engine_kwargs={"parallel": False})
```

    1.95 ms ± 115 µs per loop (mean ± std. dev. of 7 runs, 1 loop each)
...
```python
%timeit df.apply(raw_f, engine="numba", axis=1, raw=True, engine_kwargs={"parallel": True})
```

    268 µs ± 104 µs per loop (mean ± std. dev. of 7 runs, 1 loop each)
...
```python
%timeit df.sub(df.mean(axis=1), axis=0).div(df.std(ddof=0, axis=1), axis=0)
```

    2.51 ms ± 30.3 µs per loop (mean ± std. dev. of 7 runs, 100 loops each)
...
f = lambda x: x.mean()
%timeit df.apply(f, engine='numba', raw=True, engine_kwargs={'parallel': True})
%timeit df.mean(axis=1)
%timeit df.values.mean(axis=1)
```

    178 µs ± 60.9 µs per loop (mean ± std. dev. of 7 runs, 1 loop each)
    1.1 ms ± 99.8 µs per loop (mean ± std. dev. of 7 runs, 1,000 loops each)
    32.9 µs ± 2.74 µs per loop (mean ± std. dev. of 7 runs, 10,000 loops each)
...
I'd like to thank [Matthew Roeschke](https://github.com/mroeschke) for reviewing my PRs, and NASA for funding the grant to make this possible.
