...
Note, however, that Polars displays them in the original order, whereas DuckDB has rearranged the result's rows.
...
Here's an example. We start with some data about some roles in a musical theatre show, and after a dataframe operation, end up with rows in which we can combinations of "role" and "name" which never appeared in the original data.
...
The interpretation of literals, such as `pl.lit(3)`, differs a bit between Polars and SQL. In Polars, `lit(3)` means "a single row with the value of 3", whereas in SQL it would mean "repeat the value 3 row every row in this table". We can see an example of the consequence of this difference in the following example.
...
There's some other differences I'd like to touch on here, which are either quite minor or which I've written about previously.
