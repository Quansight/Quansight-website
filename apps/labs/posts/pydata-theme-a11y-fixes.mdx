...
to examine with a before-and-after lens. This will give a better idea of how
these kinds of changes can create an improved user experience for disabled
people—and often for people without disabilities, too.
...
documentation](https://pydata-sphinx-theme.readthedocumentation.io) (v0.15.4),
...
to fulfill the same function. HTML5 provides tags like header, footer, and main
that allow web page authors to break up the content on the page into meaningful
sections. These sections can then be surfaced to screen readers and other
assistive tech.
...
don't do that in your own day-to-day HTML! You can and should just use the
`<main>` tag without the role attribute.)
...
In fact, it wasn't really a button. A true HTML `<button>` is keyboard
...
There is one thing I don't love about this approach. It is a somewhat
unconventional implementation and presentation of a details/summary widget. And
let's recall that it was an unconventional use of a standard pattern (visible
label + invisible checkbox) that got us into this mess to begin with. That said,
I think this particular unconventional presentation is okay. I think it may come
across as somewhat confusing to screen reader users the first time they come
across it, but I also think the context will quickly make things clear.
...
tree](https://developer.mozilla.org/en-US/documentation/Glossary/Accessibility_tree). By
...
had low contrast, or just didn't look good. In this section, I've rounded up and
...
page that the link goes to). Many of these states can overlap. For example, a
link can be hovered and focussed simultaneously using the mouse and keyboard
together. This overlapping state is what we call a frankenstate. It's
challenging to design states in such a way that the frankenstates also look
...
you end up designing for the frakenstates in much the same way that you would
arrange a tiny apartment: carefully, thoughtfully, creatively, skillfully, and
with a willingness to make needed compromises.
...
If you find anything you want read about in a blog post like this one, please
