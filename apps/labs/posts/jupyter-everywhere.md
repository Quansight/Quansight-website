...
Thus, we went forward with a custom approach for this, based on [work implemented in the JupyterLite community](https://github.com/jupyterlite/jupyterlite/pull/1625), to add PDF export functionality using an in-browser PDF generation library called `jsPDF`.

[`jsPDF`](https://github.com/parallax/jsPDF) is a JavaScript library that allows generating PDF documents directly in the browser, without server-side processing. We integrated `jsPDF` into Jupyter Everywhere via a custom download button that converts the current notebook to a PDF and triggers its download. However, this functionality is still a work in progress. There is work to be done in improving the fidelity of the generated PDFs, especially for complex notebooks with rich visualisations, widgets, and typeset LaTeX content.
...
Popularised by platforms such as [Observable](https://observablehq.com/), Google Colaboratory, Kaggle Notebooks, and Deepnote, "Run" buttons (▶️) next to code cells are a common feature in modern interactive computing environments. These buttons provide a convenient way for users to execute individual code cells with a single click, without needing to navigate to the toolbar or use keyboard shortcuts.
...
- Real-time collaboration in JupyterLite: enabling multiple users to work on the same notebook simultaneously, with changes reflected in real-time across all users' sessions. This would involve implementing a synchronisation mechanism to keep notebooks in sync across different browser instances, through y-CRDTs (Conflict-free Replicated Data Types). It is to be noted that [previous versions of JupyterLite did include such collaboration functionality, but do not work now](https://jupyterlite.readthedocs.io/en/stable/howto/configure/rtc.html#the-current-state-of-real-time-collaboration-in-jupyterlite).
